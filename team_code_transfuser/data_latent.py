...
class CARLA_Data2(Dataset):
    ...
    def __init__(self, root, config, shared_dict=None):
        ...
        self.route_ids     = np.array(self.route_ids  ).astype(np.string_)
    ...
    def __getitem__(self, index):
        ...
        for i in range(self.seq_len):
            if not self.data_cache is None and str(measurements[i], encoding='utf-8') in self.data_cache:
                    measurements_i, images_i, lidars_i, lidars_raw_i, bevs_i = self.data_cache[str(measurements[i], encoding='utf-8')]
                ...
                if not self.data_cache is None:
                    ...
                    self.data_cache[str(measurements[i], encoding='utf-8')] = (measurements_i, compressed_imgage, lidars_i, lidars_raw_i, compressed_bevs)
        ...
        images_i = loaded_images[self.seq_len-1] # select the last in sequence
        ...
        data['speed'] = measurements[self.seq_len-1]['speed'] # used when use_velocity is set to True
